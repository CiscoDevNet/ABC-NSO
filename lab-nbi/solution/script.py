...
class Restconf(NsoConfig):
    ...
    def get_config(self, query_path=None):
        """
        Given an optional xpath filter, get the configuration from NSO via
        RESTCONF

        :param filter: (Optional) xpath filter for RESTCONF config retrieval
          filtering.
    ...
    def push_config(self, filename):
        ...
        response = self.http.post(url, json=content)
...
class Netconf(NsoConfig):
    ...
    def __init__(self):
        ...
        self.netconf_manager = manager.connect(host=self.host,
    ...
    def get_config(self, xpath_filter=None):
        ...
        nc_result = self.netconf_manager.get_config(source="running", filter=conf_filter)
    ...
    def push_config(self, filename):
        ...
        nc_result = self.netconf_manager.edit_config(target="running", config=content)
    ...
    def execute_action(self, path):
        ...
        nc_result = self.netconf_manager.dispatch(to_ele(content))
